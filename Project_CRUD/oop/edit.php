...
if(isset($_GET['update'])){
    $sql="select * from student where class='{$_GET["cla"]}' ";
    $res=$db->select($sql);
    if ($res->num_rows<=0) {
      header("location:edit.php?err=Invalid Register no..");
    }else{
...
if($update){
 ...
 ?>
                      ...
                      <form enctype='multipart/form-data' role='form' method='post' action=' ' >
                        <td><input type="text" class="form-control" id="usr" placeholder="Enter Name" name="id" value="<?php echo $result['id'] ?>"></td>
